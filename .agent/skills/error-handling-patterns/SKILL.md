...
## Workflow
1. **Identify Failure Modes**: Determine if errors are recoverable (network timeouts, input) or unrecoverable (OOM, bugs).
2. **Choose Strategy**: Select the appropriate pattern (Exceptions vs Result Types) based on the language and error type.
3. **Implement Handling**: Use the specific patterns defined below for your language.
4. **Resilience**: Apply universal patterns like Circuit Breakers for distributed systems.
5. **Review**: Check against Best Practices and Common Pitfalls.
...
- **Logging and Re-throwing:** Creates duplicate log entries (only log if handling or wrapping, or at the top level)
- **Not Cleaning Up:** Forgetting to close files, connections
- **Poor Error Messages:** "Error occurred" is not helpful
- **Returning Error Codes (where inappropriate):** Use exceptions or Result types for better safety
